...
export default function Dashboard() {
  ...
  const { expenses, incomes, totalExpenses, totalIncomes, calculateTotalPrice, categoryIcons, sendMoneys } = useDataStore(
    (state) => ({
                ...
                <div className='col-span-4 md:col-span-2 ml-5 mr-6'>
                  <div className='mt-4 ml-3'>
                    ...
                    <h2 className='text-xl font-bold mb-3'>
                      {totalExpenses && totalExpenses > totalIncomes ? `${calculateTotalPrice}` : `${calculateTotalPrice}`}
                    </h2>
                    <div className='space-y-1 mt-2 mb-1'>
                      <div className='flex p-3 border rounded border-gray-100'>
                        <div className='flex flex-wrap'>
                          <div className=' mr-5 p-2 rounded-xl bg-customEmojiBgColor'>✈️</div>
                      ...
                      <div className='flex p-3 border rounded border-gray-100'>
                        <div className='flex flex-wrap'>
                          <div className=' mr-5 p-2 rounded-xl bg-customEmojiBgColor'>💰</div>
                          <div className='mr-3 mt-1 text-xs'>
              ...
              <div className='rounded-xl h-70 flex-col justify-center items-center p-5'>
                ...
                <div className='w-full flex justify-between mt-5 text-customCardBtnTextColor'>
                  <button className='bg-customCardBtnBgColor p-2 text-xs w-5/12 rounded-lg items-center flex justify-center font-medium'>
                    <Icon icon='heroicons:plus-16-solid' className='mt-0.5 mr-1' />
                    Request
                  </button>
                  <button
                  ...
                  <FirstModal show={showFirstModal} onClose={toggleFirstModal} onContinue={handleContinue} />
                  <SecondModal show={showSecondModal} onClose={closeSecondModal} person={selectedPerson} />
                </div>
              </div>
              <div className='ml-3 p-5 border-t'>
                <div className='flex justify-between text-sm mb-2 text-center items-center'>
                  <h2 className='font-semibold'>Recent Transactions</h2>
                  <button onClick={toggleRecentModal} className='font-semibold border px-2 py-1 text-xs'>
                    Show all
                  </button>
                    ...
                    {combinedList.map((item, index) => (
                      <div className='grid grid-cols-12 border-b p-1' key={index}>
                        <div className='col-span-5 text-xs flex'>
                          <div className='mr-2'>
                            <Icon
                              ...
                              icon={item.icon.trim() ? `material-symbols:${item.icon}` : `material-symbols:home`} // or mdi:
                            />
                          </div>
                        ...
                        <div className='col-span-3 flex justify-end'>
                          <p className={`font-bold text-xs ${item.type === 'expense' ? 'text-red-600' : 'text-green-600'}`}>
                            {item.type === 'expense' ? '- $' : '+ $'}
