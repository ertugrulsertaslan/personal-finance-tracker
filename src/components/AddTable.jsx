...
export default function AddTable() {
  const { addIncome, addExpense, addCategory, categories, months } = useDataStore();
    ...
    <>
      <div className='w-full flex justify-center text-center items-center h-full'>
        <div className='w-full md:w-2/3 lg:w-1/2 bg-customLineColor p-5'>
          ...
          <div>
            <select className='h-8 w-11/12 m-5 p-1' value={type} onChange={(e) => setType(e.target.value)}>
              <option value='income'>Income</option>
              <option value='expense'>Expense</option>
            </select>
          </div>
          <div>
            <input type='text' onChange={handleIconChange} placeholder='Write icon name' className='h-8 w-11/12 m-5 p-2' />
          </div>
          <div>
            <CreatableSelect
              ...
              noOptionsMessage={() => (customInputValue.trim() === '' ? '' : null)}
            />
          </div>
          <div className='mb-5'>
          ...
          <div>
            <button className='h-8 w-48 m-5 bg-green-500 text-white' onClick={handleAddTransaction}>
              Add
            </button>
          </div>
        </div>
